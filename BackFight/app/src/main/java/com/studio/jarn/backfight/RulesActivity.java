...
public class RulesActivity extends AppCompatActivity {
    ...
    public void onMovieSelected(int position) {
        Rules selectedRule = mRules.get(position);
        if (selectedRule != null) {
            mTitle.setText(mRules.get(position).rulesName);

            String temp = "";
            for (int i = 0; i < mRules.get(position).rulesDescription.size(); i++) {
                temp = temp + mRules.get(position).rulesDescription.get(i) + System.lineSeparator();
            }
    ...
    private void setRulesInListView() {
        ...
        mSeach.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                // TODO  make search function mAdapter.getFilter().filter(s);
